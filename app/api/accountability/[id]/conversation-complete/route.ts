...
export async function POST(
  ...
  // Manager role: must be the author of the document
  ...
  await query(
    `UPDATE accountability_docs
     SET conversation_status = 'complete', email_sent_at = NOW()
     WHERE id = $1`,
    [doc.id]
  )
  ...
  await query(
    ...
    [doc.id, 'conversation_complete', session.id, session.fullName,
     `Voice conversation marked complete by ${session.fullName} — formal notice sent to employee`]
  ).catch(() => {})
