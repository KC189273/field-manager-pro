...
export async function GET() {
  ...
  // Compute date constants upfront (no DB needed)
  ...
  const todayCSTStr = today.toLocaleDateString('en-CA', { timeZone: 'America/Chicago' })

  // Run all independent queries in parallel (was 5 sequential round-trips)
  const [activeShift, schedule, currentSchedule, todayShifts] = await Promise.all([
    queryOne<{ id: string; clock_in_at: string; clock_in_lat: string; clock_in_lng: string; clock_in_address: string; store_location_id: string | null; store_address: string | null }>(
    ...
    query<{ start_time: string; end_time: string; store_address: string; role_note: string | null; break_minutes: number }>(
  ...
  // Active break requires the shift ID — runs after (dependent query)
