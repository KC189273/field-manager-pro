...
export async function GET(req: NextRequest) {
  ...
  // Get employees with 2+ late clock-ins in the period
  const lateQuery = effectiveDmId
    ? `
      SELECT f.user_id, u.full_name as employee_name, u.manager_id,
        dm.full_name as dm_name,
        COUNT(*) as late_count,
        ROUND(AVG(
          CASE WHEN f.detail ~ '\\d+ min'
            THEN (regexp_match(f.detail, '(\\d+) min'))[1]::int
            ELSE 0 END
        )) as avg_minutes_late,
        MIN(f.date) as first_late,
        MAX(f.date) as last_late
      FROM flags f
    ...
    : `
      ...
      WHERE f.type = 'late_clock_in'
        AND f.date >= CURRENT_DATE - $1
        AND u.is_active = TRUE
        ${session.org_id ? `AND u.org_id = '${(session.org_id as string).replace(/'/g, "''")}'` : ''}
      GROUP BY f.user_id, u.full_name, u.manager_id, dm.full_name
      HAVING COUNT(*) >= 2
      ORDER BY COUNT(*) DESC, u.full_name
    `

  const params = effectiveDmId ? [days, effectiveDmId] : [days]
  ...
  // Get time edit counts per employee (edits made by someone other than the employee)
  const edits = await query<{ user_id: string; edit_count: number }>(`
    ...
    WHERE s.user_id = ANY($1)
      AND se.edited_at > NOW() - ($2 || ' days')::interval
  ...
  // Get DM list for filter dropdown
  const dmQuery = canViewAll
    ? `SELECT DISTINCT u.id, u.full_name FROM users u WHERE u.role = 'manager' AND u.is_active = TRUE ${session.org_id ? `AND u.org_id = '${(session.org_id as string).replace(/'/g, "''")}'` : ''} ORDER BY u.full_name`
    : null
  const dms = dmQuery ? await query<{ id: string; full_name: string }>(dmQuery) : []

  return NextResponse.json({ offenders: result, dms })
}

// GET detail for a specific employee — their individual late dates
export async function POST(req: NextRequest) {
  ...
  // Individual late dates
  const lates = await query<{ date: string; detail: string; created_at: string }>(`
    SELECT date::text, detail, created_at::text
    FROM flags
    WHERE user_id = $1 AND type = 'late_clock_in' AND date >= CURRENT_DATE - $2
    ORDER BY date DESC
  `, [userId, days])

  // Individual time edits
