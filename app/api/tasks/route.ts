...
// DELETE /api/tasks — delete a single task or an entire recurring series
export async function DELETE(req: NextRequest) {
  ...
  if (seriesId) {
    ...
    const deleted = await query<{ id: string }>(
      `DELETE FROM tasks WHERE recurrence_id = $1 RETURNING id`,
      [seriesId]
    )
    for (const row of deleted) {
      removeTaskCalendarEvent(row.id).catch(() => {})
    }
