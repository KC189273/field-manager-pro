...
export default function ExpensesPage() {
  ...
  useEffect(() => {
    if (!session) return
    loadExpenses()
    if (session.role === 'developer' || session.role === 'owner' || session.role === 'sales_director' || session.role === 'manager' || session.role === 'ops_field_leader' || session.role === 'ops_manager') {
  ...
  const canViewDetail = session?.role === 'manager' || session?.role === 'ops_field_leader' || session?.role === 'ops_manager' || session?.role === 'owner' || session?.role === 'sales_director' || session?.role === 'developer'
