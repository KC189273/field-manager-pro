...
const ALL_FEATURES: Feature[] = [
  ...
  { href: '/dm-schedule',     label: 'DM Schedules',     short: 'DM Sched',  Icon: ScheduleIcon,       show: r => r === 'sales_director' || r === 'owner' || r === 'developer' },
  ...
  { href: '/admin/agents',    label: 'Agent Inbox',      short: 'Agents',    Icon: GearIcon,           show: r => r === 'developer' },
]
...
export default function BottomNav() {
  ...
  const [aiMessages, setAiMessages] = useState<{ id: string; role: string; body: string }[]>([])
  const [aiInput, setAiInput] = useState('')
  const [aiSending, setAiSending] = useState(false)
  const [aiConvId, setAiConvId] = useState<string | null>(null)
  const [aiStatus, setAiStatus] = useState<'new' | 'active' | 'resolved' | 'escalated'>('new')
  ...
  // Load AI chat existing conversation + user name
  useEffect(() => {
    if (!role) return
    fetch('/api/auth/me').then(r => r.ok ? r.json() : null).then(d => {
      if (d?.fullName) setAiUserName(d.fullName.split(' ')[0])
    }).catch(() => {})
    fetch('/api/support/chat').then(r => r.ok ? r.json() : null).then(d => {
      if (d?.conversation?.status === 'active') {
        setAiConvId(d.conversation.id)
        setAiStatus('active')
        setAiMessages(d.messages.filter((m: { role: string }) => m.role !== 'system').map((m: { role: string; body: string }) => ({ id: crypto.randomUUID(), ...m })))
      }
    }).catch(() => {})
  }, [role])
  ...
  function aiSend() {
    ...
    setAiSending(true)
    setAiStatus('active')
    setAiMessages(prev => [...prev, { id: crypto.randomUUID(), role: 'user', body: msg }])

    fetch('/api/support/chat', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: msg, conversationId: aiConvId }),
    }).then(r => r.json()).then(data => {
      if (data.conversationId) setAiConvId(data.conversationId)
      if (data.reply) setAiMessages(prev => [...prev, { id: crypto.randomUUID(), role: 'assistant', body: data.reply }])
      if (data.escalated) setAiStatus('escalated')
      else if (data.resolved) setAiStatus('resolved')
    }).catch(() => {
      setAiMessages(prev => [...prev, { id: crypto.randomUUID(), role: 'assistant', body: 'Connection error. Please try again.' }])
  ...
  return (
    <>
          ...
          {!aiBubbleDismissed && !aiDragging && (
            <div className="relative bg-white rounded-2xl pl-4 pr-2 py-2 shadow-lg flex items-center gap-1">
              ...
              <button
                ...
                className="w-5 h-5 rounded-full hover:bg-gray-200 flex items-center justify-center flex-shrink-0 transition-colors"
              >
              ...
              {/* Bubble tail */}
              <div className="absolute -bottom-2 right-6 w-4 h-4 bg-white rotate-45 rounded-sm shadow-lg" />
            </div>
          )}
          {/* Button */}
          <button
      ...
      {aiChatOpen && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end" onClick={() => setAiChatOpen(false)}>
          <div className="absolute inset-0 bg-black/50" />
          <div className="relative bg-gray-900 rounded-t-2xl flex flex-col" style={{ height: '80vh' }} onClick={e => e.stopPropagation()}>
            <div className="flex justify-center pt-2 pb-1"><div className="w-10 h-1 rounded-full bg-gray-700" /></div>
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b border-gray-800">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-green-400" /><p className="text-[10px] text-gray-500">Online</p></div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {(aiStatus === 'resolved' || aiStatus === 'escalated') && (
                  <button onClick={aiNewConversation} className="text-[10px] bg-violet-600 hover:bg-violet-500 text-white px-2.5 py-1 rounded-lg font-semibold">New</button>
                )}
                <button onClick={() => setAiChatOpen(false)} className="w-8 h-8 rounded-full hover:bg-gray-800 flex items-center justify-center">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
              {aiMessages.length === 0 && (
                <div className="flex justify-start">
                  <div className="max-w-[85%] bg-gray-800 rounded-2xl rounded-bl-sm px-4 py-3 text-sm text-gray-200">
                    <p className="font-medium mb-1">Hey{aiUserName ? ` ${aiUserName}` : ''}!</p>
                    <p>I&apos;m the FMP AI Assistant. Tell me what&apos;s going on and I&apos;ll help you figure it out step by step.</p>
                  </div>
                </div>
              )}
              {aiMessages.map(msg => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm ${msg.role === 'user' ? 'bg-violet-600 text-white rounded-br-sm' : 'bg-gray-800 text-gray-200 rounded-bl-sm'}`}>
              ...
              {aiStatus === 'escalated' && (
                <div className="bg-amber-900/20 border border-amber-800/40 rounded-xl px-4 py-3 text-center">
                  ...
                  <p className="text-amber-400/70 text-xs mt-1">They have the full conversation and will follow up directly.</p>
                </div>
              )}
              {aiStatus === 'resolved' && (
              ...
              <div className="px-4 py-3 border-t border-gray-800">
                <div className="flex items-end gap-2">
                  <textarea
                    ...
                    autoFocus
                  />
                  <button onClick={aiSend} disabled={!aiInput.trim() || aiSending} className="w-10 h-10 rounded-xl bg-violet-600 hover:bg-violet-500 disabled:opacity-30 flex items-center justify-center flex-shrink-0 transition-colors">
                    <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
