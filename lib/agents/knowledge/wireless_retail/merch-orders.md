...
4. Select an **ops manager** to send the order to — this is required.
...
Ops managers and above (SD, Owner, Developer) can approve merch orders. When approved, the status changes from "pending" to "ordered."
...
You must select which ops manager should handle this order from the dropdown. If no ops managers are listed, contact your SD.

## Who can see merch orders?
- **Employees/DMs** see their own orders.
- **Ops managers, SD, Owner, Developer** see all orders across the org.
