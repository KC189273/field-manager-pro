...
Employee Coached: ${params.employeeCoachedName || 'Not specified'}${params.coachingSituation ? `\nSituation: ${params.coachingSituation}` : ''}${params.coachingSituation === 'Employee on Leave / Extended Absence' ? '\n**IMPORTANT: This employee is on leave or extended absence. Do NOT penalize for lack of specific goals, timelines, or follow-up dates. Focus on whether the DM documented the situation appropriately and set expectations for when the employee returns.**' : ''}${params.coachingSituation === 'First Coaching (Territory Change)' ? '\n**IMPORTANT: This is the DMs first time coaching this employee after a territory change. Give full baseline pass on prior reference. Grade generously on depth — the DM is still learning this employees strengths and weaknesses.**' : ''}${params.coachingSituation === 'New Hire Onboarding' ? '\n**IMPORTANT: This is onboarding coaching for a new hire. Focus on whether foundational expectations were set clearly, not on advanced coaching technique. Do NOT penalize for lack of sales-specific observations or prior coaching history.**' : ''}${params.coachingSituation === 'New Store Opening' ? '\n**IMPORTANT: This is a brand new store that is just opening. Do NOT penalize for lack of prior coaching, missing sales metrics, incomplete processes, or low observation checklist scores. The store is still getting set up. Focus on whether the DM is setting clear expectations, establishing foundational habits, and building rapport with the team. Grade generously — this is about laying groundwork, not measuring established performance.**' : ''}${params.coachingSituation === 'Return from Leave' ? '\n**IMPORTANT: This employee is returning from leave. The DM should be re-establishing expectations and getting the employee back up to speed. Do NOT penalize for gaps in prior coaching continuity — the employee was absent. Focus on whether the DM set clear re-entry expectations and created a plan to bring them back to full performance.**' : ''}${params.coachingSituation === 'Performance Concern' ? '\n**IMPORTANT: This coaching is addressing a specific performance concern. The DM should be more direct and structured here. Grade based on whether the concern was clearly communicated, specific behaviors were identified, concrete improvement steps were outlined, and accountability was established. This is where coaching SHOULD be detailed and firm — reward directness, not vagueness.**' : ''}
...
4. DEPTH OF OBSERVATION (20%): How thorough was the observation? Did they use the checklist fully? Did they identify root causes, not just symptoms? One checkbox = low. Full checklist with detailed notes on each area = high. **IMPORTANT: The Role Play Notes field and Knowledge Gap Notes field are OPTIONAL. Do NOT penalize for leaving these blank. Only penalize depth if the required checklist items (observation, role play score, knowledge check ratings) are missing or minimal.**
