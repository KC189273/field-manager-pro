...
export function welcomeEmailHtml(fullName: string, username: string, password: string, role: string): string {
  ...
  const roleLabel = role === 'manager' ? 'DM' : role === 'ops_manager' ? 'Ops Manager' : role === 'ops_field_leader' ? 'Field Leader' : role === 'owner' ? 'Owner' : role === 'sales_director' ? 'Sales Director' : 'Employee'
  return `
